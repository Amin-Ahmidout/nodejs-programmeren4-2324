      ...
      chai.request(server)
          ...
          .end((err, res) => {
              ...
              chai.request(server)
                  ...
                  .send({ firstName: 'Updated', emailAdress: 't.testssssss@example.com' })
                  .end((err, res) => {
                      if (err) {
                      ...
                      expect(res.body).to.be.an('object');
                      expect(res.body).to.have.property('message').that.is.a('string');
                      expect(res.body.message).to.equal('Forbidden: You can only update your own data');
                      expect(res.body).to.have.property('data').that.is.empty;
        ...
        .end((err, res) => {
          ...
          chai.request(server)
            ...
            .end((err, res) => {
              ...
              expect(res.body.data).to.have.property('emailAdress').that.is.a('string').that.equals('t.testen@example.com');
...
describe('UC-206 Verwijderen van een user', () => {
    ...
    it.skip('TC-206-4 Gebruiker succesvol verwijderd', (done) => {
      ...
      chai.request(server)
        .post('/api/user')
        .send({ testUser }) // Add necessary user data here
        .end((err, res) => {
          ...
          expect(res.body.message).to.equal(`User created with id ${createdUserId}.`)
          ...
          chai.request(server)
            ...
            .end((err, res) => {
              ...
              expect(res.body).to.be.an('object')
              expect(res.body).to.have.property('message').that.is.a('string')
              expect(res.body.message).to.equal(`User deleted with id ${createdUserId
              }.`)
              ...
              chai.request(server)
                ...
                .end((err, res) => {
                  ...
                  expect(res.body).to.be.an('object')
                  expect(res.body).to.have.property('message').that.is.a('string')
                  expect(res.body.message).to.equal(`User with ID ${createdUserId} not found`)
                  expect(res.body).to.have.property('data').that.is.empty
                  done()
                })
            })
